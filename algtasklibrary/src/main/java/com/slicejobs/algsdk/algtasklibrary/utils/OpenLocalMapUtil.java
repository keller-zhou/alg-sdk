...
public class OpenLocalMapUtil {
     ...
     * @return
     */
    public static String getBaiduMapUri(String desLat, String desLon, String destination, String src){
        String uri = "intent://map/direction?" +
                "destination=latlng:%1$s,%2$s|name:%3$s&mode=driving&src=%4$s#Intent;" +
                "scheme=bdapp;package=com.baidu.BaiduMap;end";

        return String.format(uri, desLat, desLon, destination, src);
    }
     ...
     * @return
     */
    public static String getBaiduMapUri(String destination, String src){
        String uri = "intent://map/direction?" +
                "destination=%1$s&mode=driving&src=%2$s#Intent;" +
                "scheme=bdapp;package=com.baidu.BaiduMap;end";

        return String.format(uri, destination, src);
    }
    ...
    public static String getGdMapUri(String appName, String dlat, String dlon, String dname){
        String uri = "androidamap://route?sourceApplication=%1$s&dlat=%2$s&dlon=%3$s&dname=%4$s&dev=0&m=0&t=2";
        return String.format(uri, appName, dlat, dlon, dname);
    }

    /**
     * 获取打开高德地图应用uri
     */
    public static String getGdMapUri(String appName, String dname){
        String uri = "androidamap://route?sourceApplication=%1$s&dname=%2$s&dev=0&m=0&t=2";
        return String.format(uri, appName, dname);
    }
